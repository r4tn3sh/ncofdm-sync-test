...
int main(int argc, char * argv[]){
    ...
    const size_t num_of_samples = 2000;
...
int seq_detector(std::vector<std::complex<double> > samples)
{
    ...
    if (pnsearch%1 == 0)
    {
        pk_index = correlate_ulseq(samples);
    }
    ...
    if(pk_index < PKTLEN)
    {
        rx_time = rx_md.time_spec.get_full_secs() + rx_md.time_spec.get_frac_secs();
